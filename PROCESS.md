# 📜 프로젝트 개발 이력 및 개선 프로세스 (PROCESS.MD)

이 문서는 **AI Finance Dashboard** 프로젝트의 개발 히스토리, 주요 변경 사항, 그리고 향후 개선 계획을 추적 관리하기 위한 문서입니다.

---

## 1. 📅 개발 히스토리 (Change Log)

### [2026-02-11 ~ 02-24] 대시보드 UI 및 실시간 동기화 최적화

- **주요 변경 사항**:
  - **대시보드 Accountability 강화**: `Dashboard.jsx`의 ACCOUNT SUMMARY 표와 연간 요약 카드 내 수익금/수익률에 대해 양수(`emerald-400`)와 음수(`rose-400`) 색상 및 사이버펑크 네온 글로우(`drop-shadow`) 효과를 `!important` 구문으로 강제 적용하여 시인성을 극대화.
  - **실시간 가격 동기화 유틸리티 분리**:
    - 기존 `Portfolio.jsx`에 종속되어 있던 Finnhub, 한국투자증권, 코인게코 API 호출 및 가격 갱신 로직을 통합하여 `src/utils/priceUpdater.js` 독립 모듈로 분리.
    - `Dashboard.jsx`의 새로고침 아이콘(`RefreshCw`) 클릭 시 백그라운드에서 실시간 시장가(환율, 주식, 코인 등)를 조회하여 렌더링되게끔 최적화 (포트폴리오 탭 방문 없이도 최신 상태 보장).

### [2026-02-10] AI 리포트 (AIReport) 다크 모드 전환

- **주요 변경 사항**:
  - **리포트 히스토리 카드 다크 전환**: `renderHistory()` 내 카드 배경을 `bg-white` → `bg-slate-800/60`, 텍스트를 `text-gray-900` → `text-cyan-300`으로 변환.
  - **버튼 스타일 통일**: `전체 보기`, `복사`, `다운로드` 버튼을 `border-slate-600 hover:bg-slate-700` 사이버펑크 스타일로 전환.
  - **라이트 모드 잔재 완전 제거**: `bg-white`, `text-gray-900`, `border-gray-200`, `hover:bg-gray-50` 등 전면 제거 및 검증 완료.

### [2026-02-10] AI 모델 성능 최적화 및 기능 고도화 (Phase 2)

- **주요 변경 사항**:
  - **AI 엔진 업그레이드**: `aiService.js` 및 `constants.js` 내 AI 엔진을 `GPT-4o` (OpenAI 최신) 및 `Gemini 1.5 Pro` (Google 최신)로 전격 교체.
    - 기존 가상 미래 모델(`GPT-5.2`) 설정 제거 및 실전 최적화 모델 적용.
    - 듀얼 전략(Dual Strategy) 유지: 고급 분석(GPT-4o) / 대량 데이터 요약(Gemini 1.5).
  - **종목 분석(Stock Analysis) 탭 전면 리팩토링**:
    - **Perplexity 외부 링크 제거**: 앱 이탈을 유발하는 외부 링크 방식 폐기.
    - **Native AI 심층 분석 구현**: `GPT-4o`를 활용하여 앱 내에서 즉시 기업 개요, 투자 포인트, 리스크, 월가 전망을 생성하는 `generateStockAnalysis` 기능 구현.
    - **UI/UX 개선**: Markdown 렌더링을 통한 깔끔한 분석 결과 표시 및 복사 기능 추가.
  - **종목 분석 UI/UX 고도화**:
    - **가독성 강화**: 종목명, 가격, 수익률 등 핵심 지표의 폰트 크기 확대 및 중앙 정렬 레이아웃 적용.
    - **실시간성 보장**: 분석 기준일을 정적 연도('2025')에서 동적 현재 날짜(`new Date()`)로 변경하여 항시 최신 분석 제공.
    - **프롬프트 엔지니어링 고도화**: 단순 서술형 분석에서 벗어나 **매수/매도/홀딩**의 명확한 투자 의견, 목표 주가, 3줄 핵심 요약, 실적 전망을 포함한 구조화된 리포트(Markdown) 생성으로 변경.
    - **UI 라벨 최신화**: 'GPT-5.2', 'Gemini 3 Pro' 등 위시리스트 성격의 라벨을 실제 탑재된 **'GPT-4o'**, **'Gemini 1.5 Pro'**로 수정하고, 불필요한 'Perplexity' 문구 제거.

### [2026-02-10] 목표(Goals), 설정(Settings), 시장(Market) 다크 모드 완성

- **주요 변경 사항**:
  - **Goals.jsx**: 목표 카드의 통계 그리드, AI 제안 섹션, 모달(추가/삭제)을 `slate-900` 기반의 사이버펑크 테마로 전면 개편. 차트 툴팁 스타일 수정.
  - **Settings.jsx**: 통화 설정 버튼, API 키 입력 필드, 데이터 백업/복원 UI, 위험 영역(Danger Zone)을 다크 모드 및 네온 액센트로 통일.
  - **Market.jsx**: 암호화폐 및 환율 카드의 폴백(Fallback) 상태 디자인을 `bg-slate-900/50` 및 유색 테두리로 개선하여 일관성 확보.
  - **전체 검증**: `bg-white`, `text-gray-900` 등 라이트 모드 잔재 코드 제거 완료.

### [2026-02-10] 투자 일지 (Investment Log) 리디자인 & 배포

- **주요 변경 사항**:
  - **사이버펑크 다크 테마 적용**: `InvestmentLog.jsx` 전체 컴포넌트(리스트, 테이블, 캘린더, 모달)에 `slate-900` 배경 및 네온 컬러(`cyan`, `rose`, `emerald`) 적용.
  - **자산 명칭 표시 강화**: 테이블 뷰에서 심볼(Symbol)과 함께 자산명(Asset Name)을 병기하여 가독성 향상.
  - **캘린더 뷰 스타일링**: `react-calendar`의 기본 스타일을 오버라이딩하여 다크 모드에 최적화된 커스텀 CSS 적용.
  - **통계 카드 개선**: `MoveUp`, `MoveDown` 등의 아이콘과 네온 글로우 효과(`cyber-card-glow`)를 적용하여 시각적 완성도 제고.
  - **트랜잭션 추가 모달**: 입력 폼, 드롭다운, 버튼 등에 일관된 다크 테마 적용.

### [2026-02-10] 거래 내역 (Transaction History) 다크 모드

- **주요 변경 사항**:
  - **전면적 다크 모드 전환**: `TransactionHistory.jsx` 내 모든 모달(내역, 배당, 통계)과 섹션을 다크 테마로 전환.
  - **레거시 스타일 제거**: `bg-white`, `text-gray-*` 등 기존 라이트 모드 잔재 코드 완전 제거 및 `grep` 검증 완료.

### [2026-02-09] 자산 현황 (Asset Status) & 포트폴리오 개선

- **주요 변경 사항**:
  - **자산 현황 카드**: 수입/지출 카드에 그라디언트 글로우 효과 적용.
  - **포트폴리오 요약**: 밝은 그라디언트를 제거하고 차분한 네온 스타일로 변경.
  - **차트 페이징**: 다수의 자산을 효율적으로 표시하기 위한 차트 페이지네이션 구현.

### [2026-02-08] 글로벌 레이아웃 (Global Layout) 개편

- **주요 변경 사항**:
  - **사이버펑크 테마 기틀 마련**: `Layout.jsx` 및 `index.css`를 수정하여 전체 배경(`bg-slate-950`)과 사이드바, 헤더의 기본 스타일 정의.

---

## 2. 🚀 향후 개선 계획 (Roadmap)

### Phase 1: 잔여 UI 폴리싱 (완료)

- [x] **목표 관리 (Goals.jsx)**: 목표 카드 및 AI 제안, 모달에 사이버펑크 스타일 적용 완료.
- [x] **설정 (Settings.jsx)**: 폼 입력 필드, 통화 설정, 데이터 관리 다크 모드 전환 완료.
- [x] **시장/AI 리포트**: AI 리포트 히스토리 및 시장 데이터 폴백 UI 일관성 확보, AI 뉴스 피드 스타일 통일 완료.

### Phase 2: 기능 고도화

- [ ] **모바일 최적화**: 복잡한 데이터 테이블의 모바일 가독성 개선 (카드형 뷰 강화).
- [ ] **성능 최적화**: 대용량 데이터 차트 렌더링 속도 개선 (상태 관리 최적화).
- [ ] **AI 분석 심화**: 단순 요약을 넘어선 포트폴리오 리밸런싱 제안 알고리즘 강화.

### Phase 3: 시스템 안정성

- [ ] **테스트 커버리지 확대**: 핵심 유틸리티(`dataSync`) 및 계산 로직에 대한 단위 테스트 추가.
- [ ] **에러 핸들링**: API 오류 및 데이터 동기화 실패 시 사용자 피드백(Toast 알림) 강화.

---

## 3. 🛠 개발 원칙 및 컨벤션

### 디자인 (Design)

- **테마**: **Cyberpunk Dark** (`slate-950` 배경 기준).
- **컬러 팔레트**:
  - Main: `Slate-900` (Card/Sidebar)
  - Accent: `Cyan-400` (Info/Primary), `Rose-400` (Sell/Loss), `Emerald-400` (Buy/Profit)
- **효과**: `backdrop-blur` (Glassmorphism), `border-opacity-30` (Subtle Borders).

### 코드 (Code)

- **컴포넌트**: 기능 단위 분리 (`ChartCard`, `CyberTable` 등 재사용성 극대화).
- **데이터 관리**: `localStorage` 우선 저장 + `Supabase` 백그라운드 동기화 구조 유지.
- **검증**: UI 변경 시 반드시 모바일/데스크탑 반응형 확인 및 라이트 모드 잔재 코드 검사.
