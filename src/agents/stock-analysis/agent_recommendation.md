...
#### 👤 내 보유 종목 피드백 (Personalized Portfolio Action)
- **현재 상황 분석**: (제공된 '투자자 보유 정보'가 있다면 매수 단가와 현재 수익률을 명시하여 상황을 진단할 것. 정보가 없다면 생략)
- **맞춤형 대응 지침**: (현재 수익/손실 상태에 따른 구체적인 홀딩/추매/손절/익절 등 개인화된 조언)
...
2. 제공된 데이터 중 `[투자자 보유 정보]`가 존재할 경우, 반드시 사용자의 매수 단가와 현재가/목표가를 비교하여 개인화된 관점의 매매 조언(추매, 관망, 익절 등)을 **[내 보유 종목 피드백]**에 작성할 것
3. 추천 등급은 반드시 Bull/Bear Case 양면을 모두 제시한 후 결론
4. 목표 주가는 3단계 시나리오(낙관/기본/비관)로 제시
5. 매매 전략은 구체적인 가격 수준과 비중(%)을 명시
